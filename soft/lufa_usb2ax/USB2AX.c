...
#define RXBUFFERSIZE (AX_SYNC_READ_MAX_DEVICES*2 + 12)
uint8_t rxbyte[RXBUFFERSIZE]; // buffer where currently processed data are stored when looking for a Dynamixel packet, with enough space for longest possible sync read request
	    ...
	    for( uint8_t i = 0; i < USB_nb_received ; i++ ){
            switch (ax_state){
                ...
                case AX_SEARCH_LENGTH:
                    rxbyte[rxbyte_count++] = CDC_Device_ReceiveByte(&USB2AX_CDC_Interface);
                    if (rxbyte[PACKET_LENGTH] < (RXBUFFERSIZE-4)){  // reject message if too big for rxbyte buffer)
